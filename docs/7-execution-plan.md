...
**완료 조건**
- [ ] `POST /api/categories` — 카테고리 등록 (201, 인증 필요)
  - [ ] 이름 100자 초과 → 400
- [ ] `PATCH /api/categories/:id` — 카테고리 수정 (200, 인증 필요)
  - [ ] 기본 카테고리 수정 시도 → 422
  - [ ] 타 사용자 카테고리 접근 → 403
- [ ] `DELETE /api/categories/:id` — 카테고리 삭제 (200, 인증 필요)
  - [ ] 기본 카테고리 삭제 시도 → 422
  - [ ] 타 사용자 카테고리 접근 → 403
  - [ ] 하위 할일 기본 카테고리 재지정 확인
- [ ] `GET /api/categories` — 카테고리 목록 조회 (200, 인증 필요)
