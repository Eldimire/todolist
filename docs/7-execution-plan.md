...
**완료 조건**
- [x] US-01 흐름: 회원가입 → 로그인(JWT 수신) → 할일 등록 정상 동작
- [x] US-02 흐름: 카테고리 생성 → 할일 등록(카테고리 지정) → 카테고리별 조회 정상 동작
- [x] US-03 흐름: 진행 중 필터 조회 → 완료 처리 → 완료 필터 확인 정상 동작
- [x] US-04 흐름: 기한 초과 조회 → 날짜 수정 → 삭제 정상 동작
- [x] US-06 흐름: 내 정보 수정 → 카테고리 삭제(하위 할일 재지정) → 로그아웃 정상 동작
- [x] 모든 에러 응답이 `{ code, message }` 형식 준수 확인
- [x] JWT 없이 보호 엔드포인트 접근 시 401 반환 확인
